...
func (mq *MemberQuery) QueryUsers() *UserQuery {
...
func (mq *MemberQuery) QueryRooms() *RoomQuery {
...
func (mq *MemberQuery) WithUsers(opts ...func(*UserQuery)) *MemberQuery {
	query := (&UserClient{config: mq.config}).Query()
...
func (mq *MemberQuery) WithRooms(opts ...func(*RoomQuery)) *MemberQuery {
	query := (&RoomClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := mq.withUsers; query != nil {
		if err := mq.loadUsers(ctx, query, nodes, nil,
			func(n *Member, e *User) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withRooms; query != nil {
		if err := mq.loadRooms(ctx, query, nodes, nil,
			func(n *Member, e *Room) { n.Edges.Rooms = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemberQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Member, init func(*Member), assign func(*Member, *User)) error {
...
func (mq *MemberQuery) loadRooms(ctx context.Context, query *RoomQuery, nodes []*Member, init func(*Member), assign func(*Member, *Room)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Member)
	for i := range nodes {
		fk := nodes[i].RoomID
...
func (mq *MemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withRooms != nil {
			_spec.Node.AddColumnOnce(member.FieldRoomID)
		}
	}
