...
func (rq *RoomQuery) QueryMembers() *MemberQuery {
	query := (&MemberClient{config: rq.config}).Query()
...
// QueryRoomInfo chains the current query on the "room_info" edge.
func (rq *RoomQuery) QueryRoomInfo() *RoomInfoQuery {
	query := (&RoomInfoClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoomQuery) Clone() *RoomQuery {
	...
	return &RoomQuery{
		...
		withRoomInfo: rq.withRoomInfo.Clone(),
...
func (rq *RoomQuery) WithMembers(opts ...func(*MemberQuery)) *RoomQuery {
	query := (&MemberClient{config: rq.config}).Query()
...
// WithRoomInfo tells the query-builder to eager-load the nodes that are connected to
// the "room_info" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithRoomInfo(opts ...func(*RoomInfoQuery)) *RoomQuery {
	query := (&RoomInfoClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRoomInfo = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withMembers; query != nil {
		if err := rq.loadMembers(ctx, query, nodes,
			func(n *Room) { n.Edges.Members = []*Member{} },
			func(n *Room, e *Member) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withRoomInfo; query != nil {
		if err := rq.loadRoomInfo(ctx, query, nodes, nil,
			func(n *Room, e *RoomInfo) { n.Edges.RoomInfo = e }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoomQuery) loadMembers(ctx context.Context, query *MemberQuery, nodes []*Room, init func(*Room), assign func(*Room, *Member)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(member.FieldRoomID)
	}
	query.Where(predicate.Member(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(room.MembersColumn), fks...))
	}))
...
func (rq *RoomQuery) loadRoomInfo(ctx context.Context, query *RoomInfoQuery, nodes []*Room, init func(*Room), assign func(*Room, *RoomInfo)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "id_info" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RoomQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withRoomInfo != nil {
			_spec.Node.AddColumnOnce(room.FieldIDInfo)
		}
	}
